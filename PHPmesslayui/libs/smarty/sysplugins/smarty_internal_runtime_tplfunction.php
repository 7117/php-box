...
{
    /**
     * Call template.clas function
     *
     * @param \Smarty_Internal_Template $tpl     template.clas object
     * @param string                    $name    template.clas function name
    ...
    public function callTemplateFunction(Smarty_Internal_Template $tpl, $name, $params, $nocache)
    {
        ...
        if (isset($funcParam)) {
            ...
            // try to load template.clas function dynamically
        ...
        throw new SmartyException("Unable to find template.clas function '{$name}'");
    }

    /**
     * Register template.clas functions defined by template.clas
     *
     ...
     *                                                                                      template.clas functions defined
     *                                                                                      in template.clas
    ...
    public function registerTplFunctions(Smarty_Internal_TemplateBase $obj, $tplFunctions, $override = true)
    {
        ...
        // make sure that the template.clas functions are known in parent templates
        if ($obj->_isSubTpl()) {
    ...
    /**
     * Return source parameter array for single or all template.clas functions
     *
     * @param \Smarty_Internal_Template $tpl  template.clas object
     * @param null|string               $name template.clas function name
     *
    ...
    /**
     * Add template.clas function to cache file for nocache calls
     *
     * @param Smarty_Internal_Template $tpl
     * @param string                   $_name     template.clas function name
        ...
        if (is_file($funcParam[ 'compiled_filepath' ])) {
            ...
            if (preg_match("/\/\* {$_function} \*\/([\S\s]*?)\/\*\/ {$_function} \*\//", $code, $match)) {
                ...
                if (function_exists($_function)) {
                    // search cache file template.clas
                    $tplPtr = $tpl;
                    while (!isset($tplPtr->cached) && isset($tplPtr->parent)) {
                        $tplPtr = $tplPtr->parent;
                    }
                    // add template.clas function code to cache file
    ...
    /**
     * Save current template.clas variables on stack
     *
     * @param \Smarty_Internal_Template $tpl
    ...
    /**
     * Restore saved variables into template.clas objects
     *
     * @param \Smarty_Internal_Template $tpl
     * @param string                    $name stack name
     */
