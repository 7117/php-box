...
class Smarty_Internal_Compile_Section extends Smarty_Internal_Compile_Private_ForeachSection
{
    ...
    public $nameProperties = array(
        'first', 'last', 'IndexController.class', 'iteration', 'show', 'total', 'rownum', 'index_prev',
        'index_next', 'loop'
    );
    ...
    public function compile($args, Smarty_Internal_TemplateCompilerBase $compiler)
    {
        ...
        $cmpFor = array();
        $propValue = array(
            'IndexController.class' => "{$sectionVar}->value['index']", 'show' => 'true', 'step' => 1,
            'iteration' => "{$local}iteration",
        );
        $propType = array('IndexController.class' => 2, 'iteration' => 2, 'show' => 0, 'step' => 0,);
        ...
        $namedAttr['IndexController.class'] = true;
        $output = "<?php\n";
        ...
        if ($propType[ 'step' ] === 0) {
            if ($propValue[ 'step' ] === 1) {
                $incFor['IndexController.class'] = "{$sectionVar}->value['index']++";
            } elseif ($propValue[ 'step' ] > 1) {
                $incFor['IndexController.class'] = "{$sectionVar}->value['index'] += {$propValue['step']}";
            } else {
                $incFor['IndexController.class'] = "{$sectionVar}->value['index'] -= " . -$propValue[ 'step' ];
            }
        } else {
        ...
        $initFor['IndexController.class'] = "{$sectionVar}->value['index'] = {$propValue['start']}";
        ...
        if (isset($namedAttr[ 'index_prev' ])) {
            $output .= "{$sectionVar}->value['index_prev'] = {$propValue['IndexController.class']} - {$propValue['step']};\n";
        }
        if (isset($namedAttr[ 'index_next' ])) {
            $output .= "{$sectionVar}->value['index_next'] = {$propValue['IndexController.class']} + {$propValue['step']};\n";
        }
