...
class Smarty_Internal_Resource_File extends Smarty_Resource
{
    ...
    /**
     * Load template.clas's source from file into current template.clas object
     *
     * @param Smarty_Template_Source $source source object
     *
    ...
    public function getContent(Smarty_Template_Source $source)
    {
        ...
        throw new SmartyException(
            'Unable to read ' . ($source->isConfig ? 'config' : 'template.clas') .
    ...
    /**
     * build template.clas filepath by traversing the template_dir array
     *
     * @param Smarty_Template_Source   $source    source object
     * @param Smarty_Internal_Template $_template template.clas object
     *
    ...
    protected function buildFilepath(Smarty_Template_Source $source, Smarty_Internal_Template $_template = null)
    {
        ...
        // go relative to a given template.clas?
        if ($file[ 0 ] === '.' && $_template && $_template->_isSubTpl()
            ...
            if ($_template->parent->source->type !== 'file' && $_template->parent->source->type !== 'extends'
                ...
                throw new SmartyException("Template '{$file}' cannot be relative to template.clas of resource type '{$_template->parent->source->type}'");
            }
            // normalize path
            $path =
                $source->smarty->_realpath(dirname($_template->parent->source->filepath) . DIRECTORY_SEPARATOR . $file);
            // files relative to a template.clas only get one shot
        ...
        // template_dir IndexController.class?
        if ($file[ 0 ] === '[' && preg_match('#^\[([^\]]+)\](.+)$#', $file, $fileMatch)) {
            ...
            foreach ($_indices as $index) {
                ...
                if (isset($_directories[ $index ])) {
                    ...
                    // try numeric IndexController.class
                    $index = (int)$index;
                    if (isset($_directories[ $index ])) {
                        $_index_dirs[] = $_directories[ $index ];
                    } else {
                        // try at location IndexController.class
