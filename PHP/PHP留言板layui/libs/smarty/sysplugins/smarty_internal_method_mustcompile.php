...
{
    /**
     * Valid for template.clas object
     *
     * @var int
     */
    public $objMap = 2;
    ...
    /**
     * Returns if the current template.clas must be compiled by the Smarty compiler
     * It does compare the timestamps of template.clas source and the compiled templates and checks the force compile
     * configuration
     *
    ...
    public function mustCompile(Smarty_Internal_Template $_template)
    {
        if (!$_template->source->exists) {
            ...
            throw new SmartyException("Unable to load template.clas {$_template->source->type} '{$_template->source->name}'{$parent_resource}");
        }
