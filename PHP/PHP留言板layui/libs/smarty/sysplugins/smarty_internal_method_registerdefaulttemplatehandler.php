...
class Smarty_Internal_Method_RegisterDefaultTemplateHandler
{
    ...
    /**
     * Register template.clas default handler
     *
    ...
    public function registerDefaultTemplateHandler(Smarty_Internal_TemplateBase $obj, $callback)
    {
        ...
        if (is_callable($callback)) {
            ...
            throw new SmartyException('Default template.clas handler not callable');
        }
        return $obj;
    }

    /**
     * get default content from template.clas or config resource handler
     *
        ...
        if (is_string($_return)) {
            ...
            if ($source->exists) {
                ...
                throw new SmartyException(
                    ...
                    ($source->isConfig ? 'config' : 'template.clas') .
            ...
            $source->exists = false;
            throw new SmartyException(
                'Default handler: No ' . ($source->isConfig ? 'config' : 'template.clas') .
