...
class Smarty_Internal_Method_GetTags
{
    ...
    /**
     * Return array of tag/attributes of all tags used by an template.clas
     *
     * @api  Smarty::getTags()
    ...
    public function getTags(Smarty_Internal_TemplateBase $obj, $template = null)
    {
        ...
        if ($obj->_isTplObj() && !isset($template)) {
            ...
            if (!$tpl->source->exists) {
                throw new SmartyException("Unable to load template.clas {$tpl->source->type} '{$tpl->source->name}'");
            }
        }
        if (isset($tpl)) {
        ...
        throw new SmartyException('Missing template.clas specification');
    }
}
