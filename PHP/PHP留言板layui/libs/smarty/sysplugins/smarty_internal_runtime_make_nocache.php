...
{
    /**
     * Save current variable value while rendering compiled template.clas and inject nocache code to
     * assign variable value in cahed template.clas
     *
    ...
    public function save(Smarty_Internal_Template $tpl, $var)
    {
        if (isset($tpl->tpl_vars[ $var ])) {
            ...
            if (preg_match('/(\w+)::__set_state/', $export, $match)) {
                throw new SmartyException("{make_nocache \${$var}} in template.clas '{$tpl->source->name}': variable does contain object '{$match[1]}' not implementing method '__set_state'");
            }
    ...
    /**
     * Store variable value saved while rendering compiled template.clas in cached template.clas context
     *
     * @param \Smarty_Internal_Template $tpl
     * @param string                    $var variable name
